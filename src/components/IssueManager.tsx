...
export default function IssueManager({ errors, warnings, info }: IssueManagerProps) {
  ...
  // Filter issues by search query and severity
  const filteredIssues = useMemo(() => {
    let filtered = allIssues;

    // Filter by severity (show only critical/warning, exclude info)
    if (showOnlyIssues) {
      filtered = filtered.filter(issue => issue.severity === 'critical' || issue.severity === 'warning');
    }
    ...
    return filtered;
  }, [allIssues, searchQuery, showOnlyIssues]);
                    ...
                    <div className="space-y-3 animate-slide-down">
                      {issues.map((issue, idx) => (
                        <div key={`${groupName}-${idx}`} className="animate-fade-in" style={{ animationDelay: `${idx * 0.05}s`, opacity: 0, animationFillMode: 'forwards' }}>
                          <IssueCard issue={issue} searchQuery={searchQuery} />
                        </div>
                      ))}
                    </div>
                  )}
